...
def generate_clips_from_file(input_filename, output_filename):
    ...
    with open(input_filename, 'r', encoding='utf-8') as infile:
        for line in infile:
            ...
            # Парсим строку вида: id;ids;ids;Названия -> Результат
            # r061;f46 f06 f02;f66;Тесто... + Дрожжи... -> Тесто для пиццы...
            parts = line.split(';')
            if len(parts) < 4:
                continue
            
            rule_id = parts[0]
            recipe_content = parts[3]
            ...
            # Случайный коэффициент правила от 0.0 до 1.0
            coeff = round(random.uniform(0.1, 1.0), 2)
            
            # Сборка шаблона правила
            rule_text = (
                ...
                f"        (assert (sendmessage (value (str-cat \"ПРОЦЕСС: {result_product} CF=\" ?res))))\n"
                f"    )\n"
                f")\n"
            )
